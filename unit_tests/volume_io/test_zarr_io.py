...
@pytest.mark.parametrize(
    ...
    [
        ...
        (b'{"zarr_format": 2}', b'{"multiscales": []}', AssertionError),
        (
            b'{"zarr_format": 2}',
            b'{"multiscales": [{"foo": "bar"}]}',
            AssertionError,
        ),
        (
            b'{"zarr_format": 2}',
            b'{"multiscales": [{"datasets": [{"path": "0"}]}]}',
            None,
        ),
        # (b'{"zarr_format": 2}', b'{"multiscales": [{"path": "0"}]}', None),
    ],
)
...
def test_zarray_info_dict_vary_fetch_file(
    ...
    def mock_fetch(*args, **kwargs):
        ...
        counter += 1
        if isinstance(return_value, type) and issubclass(
...
def test_zarrio_roundtrip(tmpdir):
    ...
    dummy_chunk = np.arange(8 * 8 * 8, dtype="uint16").reshape(1, 8, 8, 8)
    io = ZarrV2IO(accessor)
    chunk_coord = (0, 8, 16, 24, 8, 16)
    io.write_chunk(dummy_chunk, "foo0", chunk_coord)

    retrieved_chunk = io.read_chunk("foo0", chunk_coord)
